...
{
    public partial class ViewClothes : Form
    {
        // Fields
        ...
        public ViewClothes(SalesItems parentForm)
        {
            InitializeComponent();
            CheckNegative();
            CheckBelowStock();
        ...
        private void LoadSizeOptions()
        {
            ...
            try
            {
                itemSize = Program.itemSpec.Select("item_id = " + itemId, " size_order ASC").CopyToDataTable();
            }
            catch (Exception e)
            {
                MessageBox.Show("No size options");
                return;
            }
        ...
        private void ToggleAdd()
        {
            if (quantity != 0)
            {
                buttonsViewClothesAdd.Enabled = true;
                buttonsViewClothesAdd.BackgroundColor = Color.FromName("white");
            } else
            {
                buttonsViewClothesAdd.Enabled = false;
                buttonsViewClothesAdd.BackgroundColor = Color.FromName("Gainsboro");
            }
        }
        ...
        private void buttonsViewClothesPlus_Click(object sender, EventArgs e)
        {
            quantity ++;
            subtotal += price;
            CheckNegative();
        ...
        private void buttonsViewClothesMinus_Click(object sender, EventArgs e)
        {
            quantity--;
            subtotal -= price;
            CheckNegative();
        ...
        private void buttonsViewClothesAdd_Click(object sender, EventArgs e)
        {
            ...
            foreach (SalesBills bills in _parent._parent.panelBills.Controls)
            {
            ...
            SalesBills newBill = new SalesBills(_parent._parent);
            ...
            this._parent._parent.panelBills.Controls.Add(newBill);
            POS1.Forms.SalesForm.billsList.Add(newBill);
        }
